...
export const WalletModal = ({ isOpen, onClose }: WalletModalProps) => {
  const { connectWallet, isConnecting, error: contextError, address } = useWallet()
    ...
    if (isOpen && typeof window !== "undefined") {
        const checkWalletAvailability = () => {
            const availability: WalletAvailability = {
              ...
              metamask: typeof window !== "undefined" && !!(window as any).starknetMetaMask,
            }
        ...
        <div className="p-4">
          ...
          <div className="space-y-2 mb-6">
            {wallets.map((wallet) => (
              <WalletOption
                ...
                disabled={
                    walletAvailability[wallet.id as keyof WalletAvailability] === false ||
          ...
          <div className="relative mb-4">
            ...
            <div className="flex justify-center -mt-2.5">
              <span className="bg-black px-3 text-xs text-gray-400">Onchain Features</span>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            {features.map((feature) => (
              <FeatureCard key={feature.id} name={feature.name} icon={feature.icon} />
            ))}
          </div>
        </div>
      </div>
